...
> **Note on Arithmetic:** Integer arithmetic operations (`Add`, `Sub`, `Mul`, `Div`) are **fully implemented at the VM/bytecode level** and work correctly (see `crates/tksvm/tests/vm.rs` and `vm_oop.rs` for passing tests). However, the **parser does not currently support infix arithmetic syntax** for integer expressions (e.g., `self.value + delta`). The `+`, `-`, `*` operators are only parsed within ordinal expressions (e.g., `omega + 1`). For integer arithmetic in TKS source, use FFI functions or construct bytecode programmatically via the IR. Full infix integer arithmetic syntax for the parser is planned.
...
| `get doubled() { return x*2; }` | `details { doubled: Int = identity.x; }` | Computed property (parser arithmetic planned) |
...
### What Works (VM/Bytecode Level)

Integer arithmetic is **fully functional at the bytecode/VM level**:

| Opcode | Operation | Example (bytecode) |
|--------|-----------|-------------------|
| `Add` | Addition | `PushInt 2; PushInt 3; Add` -> `5` |
| `Sub` | Subtraction | `PushInt 5; PushInt 2; Sub` -> `3` |
| `Mul` | Multiplication | `PushInt 4; PushInt 3; Mul` -> `12` |
| `Div` | Division | `PushInt 10; PushInt 2; Div` -> `5` |

These are tested and passing in:
- `crates/tksvm/tests/vm.rs` - basic arithmetic tests (`run_add_program`)
- `crates/tksvm/tests/vm_oop.rs` - arithmetic within OOP patterns (`run_record_with_arithmetic_method` tests `Mul`)

### Current Parser Limitation

The parser currently only supports `+`, `-`, `*` for **ordinal arithmetic**:

```tks
omega + 1       -- Works: ordinal addition
omega * omega   -- Works: ordinal multiplication
```

Integer infix expressions are **not yet parsed**:

```tks
let x = 1 + 2;  -- Does NOT work: parser error
```

### Workarounds

1. **Use FFI**: Call external functions for arithmetic
2. **Direct IR/bytecode**: Construct `IRTerm::IntOp` or bytecode instructions programmatically
3. **Wait for parser update**: Infix integer arithmetic syntax is planned
...
Methods can return RPM values (conceptual example - division operator is a planned parser extension):
...
- **Integer arithmetic syntax**: `+`, `-`, `*`, `/` operators for Int expressions in parser
...
8. **Integer arithmetic works at VM level**; parser syntax is planned
'''

with open('TKS_OOP_HYBRID.md', 'w', encoding='utf-8') as f:
    f.write(content)
print('Updated TKS_OOP_HYBRID.md')
