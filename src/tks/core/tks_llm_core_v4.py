...
class CausalFractalAttentionMechanism(nn.Module):
    ...
    def forward(
        ...
        for s in range(self.num_scales):
            ...
            # Use -1e4 for masking (safe for FP16, -65504 is FP16 min)
            mask_value = -1e4
            ...
            # Key padding mask: prevent attending to PAD positions (applied BEFORE softmax)
            if attention_mask is not None:
                # attention_mask: [batch, seq] where 1=real, 0=PAD
                # For downsampled keys, we need to downsample the mask too
                key_mask = attention_mask[:, ::factor] if factor > 1 else attention_mask
                # Expand to [batch, 1, key_len] for broadcasting with [batch, seq, key_len]
                key_mask = key_mask.unsqueeze(1).float()
                attn_scores = attn_scores.masked_fill(key_mask == 0, mask_value)
...
class TKSNoeticLM(nn.Module):
    ...
    def check_governance(
        ...
        **kwargs,
        ...
        return self.governance.check_action(
            action=action,
