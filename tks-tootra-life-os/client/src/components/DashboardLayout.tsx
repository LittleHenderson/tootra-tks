...
function DashboardLayoutContent({
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (detail?.path) setLocation(detail.path);
    };
    window.addEventListener("tks-tour-navigate", handler);
    return () => window.removeEventListener("tks-tour-navigate", handler);
  }, [setLocation]);
      ...
      <div className="relative" ref={sidebarRef}>
        <Sidebar collapsible="icon" className="border-r-0" disableTransition={isResizing}>
          ...
          <SidebarContent className="gap-0">
            <SidebarMenu className="px-2 py-1" data-tour="sidebar-nav">
            ...
            {/* Mode Toggle */}
            {!isCollapsed && (
              <div className="px-4 mt-4" data-tour="mode-toggle">
                <button
