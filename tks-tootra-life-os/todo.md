...
## GitHub Push
- [x] Push latest code to LittleHenderson/tootra-tks repo

## Interactive Tour Mode
- [x] Build TourMode component with highlight overlay + tooltip on actual UI elements
- [x] Tour Step 1: Highlight the task capture input area with tooltip "Type your task here"
- [x] Tour Step 2: Highlight the Capture & Map button with tooltip "Click to let AI analyze your task"
- [x] Tour Step 3: Highlight the Inbox section with tooltip "Your captured tasks appear here"
- [x] Tour Step 4: Highlight the sidebar navigation with tooltip "Explore all TKS features from here"
- [x] Tour Step 5: Highlight the Scenarioize button with tooltip "Visualize your future outcomes"
- [x] Tour Step 6: Highlight the mode toggle with tooltip "Switch between Life OS and Work Sprint"
- [x] Add "Start Tour" option at end of onboarding slideshow
- [x] Add "Take a Tour" button accessible from sidebar or How to Use page
- [x] Tour auto-advances when user clicks Next or clicks the spotlight overlay
- [x] Tour can be dismissed at any step
- [x] Persist tour completion state in localStorage
- [x] Write vitest tests for Tour Mode component logic (31 tests passing)

## Action-Triggered Tour Auto-Advance
- [x] Tour Step 1 (capture-input): auto-advance when user types in the textarea
- [x] Tour Step 2 (capture-button): auto-advance when user clicks the Capture & Map button
- [x] Tour Step 3 (inbox-section): auto-advance when user clicks on any inbox task card
- [x] Tour Step 4 (sidebar-nav): auto-advance when user clicks any sidebar nav item
- [x] Tour Step 5 (scenarioize-btn): auto-advance when user clicks a Scenarioize button
- [x] Tour Step 6 (mode-toggle): auto-advance when user clicks the mode toggle
- [x] Add visual hint on each step indicating the expected action (e.g., "Try it!" badge)

## Conditional First-Time Tour Auto-Launch
- [x] Auto-launch tour for returning users who completed onboarding but haven't done the tour
- [x] Ensure tour doesn't conflict with onboarding (only one shows at a time)
...
- [x] Save final checkpoint for publishing (version: fe7792e1)
- [x] Push latest code to GitHub (LittleHenderson/tootra-tks)

## TKS Inversion Engine (29 Modes)
- [x] Define all 29 inversion modes as shared constants in shared/inversions.ts
- [x] Include 4 layers: Surface (6), Deep Structural (12), Meta-layer (7), Special (4)
- [x] Define axis grid (Mode x Axis) for each inversion mode
- [x] Define preset recipes (Shadow Self, Material Mirror, Emotional Reversal, etc.)
- [x] Add database table for storing inversion results
- [x] Create server-side inversion tRPC procedure with LLM integration
- [x] Build Inversion Dial UI component with mode selector grouped by layer
- [x] Add intensity controls (soft/medium/hard) and scope selector
- [x] Add preset recipe quick-select buttons
- [x] Integrate Inversion Dial into Scenarioize component
- [x] Display inverted equation + narrative alongside original
