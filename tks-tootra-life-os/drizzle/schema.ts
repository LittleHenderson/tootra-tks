...
export const inversions = mysqlTable("inversions", {
  id: int("id").autoincrement().primaryKey(),
  taskId: int("taskId").notNull(),
  userId: int("userId").notNull(),
  // Inversion config
  modeIds: json("modeIds").$type<number[]>().notNull(),
  intensity: varchar("intensity", { length: 16 }).notNull(),
  scope: varchar("scope", { length: 16 }).notNull(),
  presetId: varchar("presetId", { length: 64 }),
  // Original
  originalEquation: text("originalEquation"),
  originalNarrative: text("originalNarrative"),
  // Inverted
  invertedEquation: text("invertedEquation"),
  invertedNarrative: text("invertedNarrative"),
  changeSummary: json("changeSummary").$type<string[]>(),
  // Inverted TKS properties
  invertedFoundationId: int("invertedFoundationId"),
  invertedArchetype: varchar("invertedArchetype", { length: 32 }),
  invertedDesireStage: varchar("invertedDesireStage", { length: 32 }),
  invertedWisdomStage: varchar("invertedWisdomStage", { length: 32 }),
  invertedPowerStage: varchar("invertedPowerStage", { length: 32 }),
  // Scenario
  scenarioPrompt: text("scenarioPrompt"),
  imageUrl: text("imageUrl"),
  // Metadata
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type Inversion = typeof inversions.$inferSelect;
export type InsertInversion = typeof inversions.$inferInsert;
