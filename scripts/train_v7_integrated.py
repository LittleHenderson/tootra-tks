#!/usr/bin/env python3
"""
TKS v7 Integrated Reasoning Training - Stage 6
Trains TKSGeneralLMv7 on "Trace -> Answer" Chain-of-Thought Data.
Goal: Bridge deep reasoning with coherent output generation.
"""
...
class IntegratedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: Tokenizer, max_seq_len: int = 512):
        ...
        print(f"Loaded {len(self.data)} integrated samples")
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        # Format: Input -> Output (Trace is part of the output string)
...
def train(args):
    ...
    # Config - Re-enable full depth for reasoning power
    ...
    print(f"Starting Stage 6 Integrated Reasoning: {len(dataset)} samples")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            with torch.autocast(device_type='cuda', dtype=AMP_DTYPE):
                ...
                # Balanced Loss: Reason deeply but speak clearly
                dps = outputs['dps_state']
                depth_reward = 0.05 * dps.p_max # Moderate reward
    ...
    torch.save(model.state_dict(), f"{args.output_dir}/v7_integrated_model.pt")
    print(f"Integrated Model saved to {args.output_dir}/v7_integrated_model.pt")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='data/v7_integrated_training.jsonl')
    parser.add_argument('--output-dir', default='checkpoints/v7_integrated')
