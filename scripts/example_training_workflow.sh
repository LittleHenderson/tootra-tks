#!/bin/bash
# TKS Training Pipeline - Example Workflow
# This script demonstrates a complete training workflow from data generation to model training
...
# Configuration
INPUT_DATA="data/pilot/stories.jsonl"
OUTPUT_DIR="output/example_training"
AUGMENTED_DATA="${OUTPUT_DIR}/augmented_corpus.jsonl"

# Step 1: Generate augmented data (if not already generated)
...
if [ ! -f "$AUGMENTED_DATA" ]; then
    python scripts/generate_augmented_data.py \
        ...
        --validate \
        --verbose
    echo "  [✓] Augmented data generated: $AUGMENTED_DATA"
else
    echo "  [✓] Using existing augmented data: $AUGMENTED_DATA"
fi
echo ""

# Step 2: Run smoke test to validate data pipeline
...
# Step 3: Dry-run to validate training pipeline
...
# Step 4: Full training run (small scale)
...
python scripts/train_with_augmented.py \
    ...
    --log-interval 10 \
    --seed 42 \
    --output-dir "${OUTPUT_DIR}/training_run_1"
echo ""

# Step 5: Advanced training with metadata
...
python scripts/train_with_augmented.py \
    ...
    --log-interval 5 \
    --seed 123 \
    --output-dir "${OUTPUT_DIR}/training_run_2"
echo ""

# Step 6: Early stopping experiment
...
echo "  - Training run 1:    ${OUTPUT_DIR}/training_run_1/metrics/"
echo "  - Training run 2:    ${OUTPUT_DIR}/training_run_2/metrics/"
