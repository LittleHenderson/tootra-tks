#!/usr/bin/env python3
"""
TKS v7 Inversion Decision Training - Project MAJIK
Trains TKSGeneralLMv7 on Inversion Decision Data (when to invert/chain).
"""
...
class InversionDataset(Dataset):
    """Dataset for v7 Inversion Decision Training."""
            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        continue
        print(f"Loaded {len(self.data)} inversion decision samples")
    ...
    def __getitem__(self, idx):
        ...
        # Format: Input -> Decision -> Reasoning -> Output
        # This teaches the model the decision process
        
        # Construct full text from available fields
        parts = [f"Input: {item.get('input', '')}"]
        ...
        if 'tks_before' in item and 'tks_after' in item:
             parts.append(f"TKS: {item['tks_before']} -> {item['tks_after']}")
...
def train(args):
    ...
    # Init v7 Model - High novelty sensitivity for decision making
    config = TKSGeneralConfigV7(
        ...
        dps_heavy_threshold=0.01, # Keep ultra-low for initial training
    ...
    print("Initialized TKSGeneralLMv7 with DPS (Inversion Mode)")

    # Dataset
    full_dataset = InversionDataset(args.data, tokenizer)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # Debug: show novelty history if available
            nw_info = ""
            ...
            if batch_idx % 10 == 0: # More frequent logs for small dataset
                print(f"Epoch {epoch+1} | Batch {batch_idx}/{len(train_loader)} | "
                      f"Loss: {loss.item():.4f} (CE: {ce_loss.item():.4f}) | "
                      f"Depth: {dps_state.p_max} | Tokens: {dps_state.tokens}{nw_info}")
    ...
    torch.save(model.state_dict(), f"{args.output_dir}/v7_inversion_model.pt")
    print(f"Saved model to {args.output_dir}/v7_inversion_model.pt")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='data/v7_inversion_decision_training.jsonl')
    parser.add_argument('--output-dir', default='checkpoints/v7_inversion')
    parser.add_argument('--epochs', type=int, default=10) # More epochs for smaller dataset
