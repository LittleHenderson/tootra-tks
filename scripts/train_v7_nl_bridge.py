#!/usr/bin/env python3
"""
TKS v7 Coherent Reasoning Training - Stage 5
Trains TKSGeneralLMv7 on Natural Language Bridge Data.
Goal: Fix "word salad" by teaching grammatical explanation structure.
"""
...
class NLBridgeDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: Tokenizer, max_seq_len: int = 512):
        ...
        print(f"Loaded {len(self.data)} NL Bridge samples")
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        # Format: Input -> Output (No trace, direct English)
...
def train(args):
    ...
    # Config - Reduce depth rewards to focus on surface fluency
    ...
    prev_path = "checkpoints/v7_stabilized/v7_final_stabilized.pt"
    ...
    dataset = NLBridgeDataset(args.data, tokenizer)
    ...
    print(f"Starting Stage 5 NL Bridge: {len(dataset)} samples")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            with torch.autocast(device_type='cuda', dtype=AMP_DTYPE):
                ...
                # PURE CE LOSS - focus on language modeling, ignore depth rewards
    ...
    torch.save(model.state_dict(), f"{args.output_dir}/v7_coherent_model.pt")
    print(f"Coherent Model saved to {args.output_dir}/v7_coherent_model.pt")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='data/v7_nl_bridge_training.jsonl')
    parser.add_argument('--output-dir', default='checkpoints/v7_coherent')
    parser.add_argument('--epochs', type=int, default=3)
    parser.add_argument('--batch-size', type=int, default=16)
    parser.add_argument('--lr', type=float, default=2e-4) # Slightly higher LR to break bad patterns
